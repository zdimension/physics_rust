use bevy::prelude::{Commands, Component, Entity, EventWriter, info, Parent, Query, Res, With};
        ...
        for (id, parent, mut initial_pos) in wnds.iter_mut() {
            egui::Window::new("Geom actions")
                ...
                .subwindow(id, ctx, &mut initial_pos, &mut commands, |ui, _commands| {
                    if ui.add(egui::Button::image_and_text(gui_icons.hinge, [16.0, 16.0], "Add center axle"))
                        .clicked() {
                        info!("Add center axle {:?}", parent.get());
                        add_obj.send(AddObjectEvent::Hinge(AddHingeEvent::AddCenter(parent.get())));
                    }
                });
        }
    }
