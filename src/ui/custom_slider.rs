...
/// Specifies the orientation of a [`CustomSlider`].
...
/// If you want to clamp incoming and outgoing values, use [`CustomSlider::clamp_to_range`].
///
...
/// ```
///
/// The default [`CustomSlider`] size is set by [`crate::style::Spacing::slider_width`].
...
impl<'a> Widget for CustomSlider<'a> {
    fn ui(mut self, ui: &mut Ui) -> Response {
        let inner_response = egui::Frame::group(ui.style())
            ...
            .show(ui, |ui| {
                ui.allocate_ui_with_layout(egui::vec2(0.0, 40.0), Layout::bottom_up(Align::Min), |ui| {
